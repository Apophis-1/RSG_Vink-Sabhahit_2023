...
! ***********************************************************************
      ...
      subroutine my_wind(id, L_phot, M_phot, R_surf, T_phot, w, ierr)
         ...
         real(dp) ::  L1, M1, T1, R1, X, Y, Z, Z_Fe, w_vink                     ! surface properties
         real(dp) ::  Z_init, X_c_init, Y_c_init, Zsolar, const_k, X_c          
         real(dp) ::  vesc, vesc_eff, vinf, vinf_fac, Teff_jump, alpha
         real(dp) ::  w_all, w_RSG, w_WR, alfa_WR, mass_loss, Gamma_e, wlow, whigh, w1, w2
         ...
         Z_Fe = Z_init
         X_c = s% xa(s%net_iso(ih1),nz)        ! to check condition to switch to WR recipe from SV2020
         Y_c_init = 0.24 + 2*Z_init
         X_c_init = 1 - Y_c_init - Z_init
         Gamma_e = 10**(-4.813)*(1+X)*(L1/M1)*(Msun/Lsun)
         ...
         if (X_c/X_c_init > 0.999) then               ! mass loss during PMS is set to zero
              mass_loss = 0
              flag = 0
         else if (X_c/X_c_init < 0.999) then
              if (T1 > 5d3 .and. T1 < 1d5) then
         	   ...
         	   call eval_high_Gamma_Vink(whigh)
                   ...
                   mass_loss = s% Dutch_scaling_factor * MAX(wlow, whigh)
              else if (T1 < 4d3) then
                   call eval_RSG_wind(w)
                   mass_loss = w
                   flag = 3
              else if (T1 > 4d3 .and. T1 < 5d3) then
                   call eval_low_Gamma_Vink(wlow)
         	   call eval_high_Gamma_Vink(whigh)
                   call eval_RSG_wind(w)

                   w1 = s% Dutch_scaling_factor * MAX(wlow, whigh)
                   w2 = w

                   alpha = (T1 - 4d3)/(1d3)
                   mass_loss = w1*alpha + w2*(1-alpha)
              else if (T1 > 1d5) then
         ...
         subroutine eval_sv2020_WR_mdot(w)
            ...
            a = 2.932
            c = -0.44*log10_cr(Z_Fe/Zsolar) + 9.15
            gamma_eb = -0.324*log10_cr(Z_Fe/Zsolar) + 0.244
            log10w_off = 0.23*log10_cr(Z_Fe/Zsolar) - 2.61
         ...
         subroutine eval_RSG_wind(w)
              
            real(dp), intent(out) :: w
            real(dp) :: log10w, Mdot_low, Mdot_high
            include 'formats'
            ...
            if (Mdot_low > Mdot_high) then
                write(*,*) 'RSG mass loss scaling similar to Reimers 1975'
            else
                write(*,*) 'RSG mass loss scaling similar to Beasor 2020'
            end if

            w = exp10_cr(log10w)
         end subroutine eval_RSG_wind



         subroutine eval_low_Gamma_Vink(wlow)
            real(dp), intent(inout) :: wlow
            real(dp) :: logMdot, vinf_div_vesc, wind
            ...
            wind = exp10_cr(logMdot)

            wlow = wind
         end subroutine eval_low_Gamma_Vink


         subroutine eval_high_Gamma_Vink(whigh)
